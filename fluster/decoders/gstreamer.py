...
@register_decoder
class FluendoH264VADecoder(GStreamerVideo):
    """Fluendo H.264 hardware decoder implementation for GStreamer"""
...
@register_decoder
class FluendoFluVAH265DecDecoder(GStreamerVideo):
    """Fluendo H.265 separated plugin hardware decoder for GStreamer"""

    codec = Codec.H265
    decoder_bin = " fluhwvah265dec "
    provider = "Fluendo"
    api = "HW"
    name = f"{provider}-{codec.value}-{api}-hwvah265dec"


@register_decoder
class FluendoFluVAH264DecDecoder(GStreamerVideo):
    """Fluendo H.264 separated plugin hardware decoder for GStreamer"""

    codec = Codec.H264
    decoder_bin = " fluhwvah264dec "
    provider = "Fluendo"
    api = "HW"
    name = f"{provider}-{codec.value}-{api}-hwvah264dec"
