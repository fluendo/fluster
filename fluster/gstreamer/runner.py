...
class GStreamerRunner:
    ...
    def _is_not_supported_error(self, error_domain: int, error_code: int) -> bool:
        """Return whether a Gst error maps to an unsupported decoder result."""
        if self.gst is None:
            return False

        if error_domain == self.gst.stream_error_quark():
            return error_code in (GST_STREAM_ERROR_CODEC_NOT_FOUND, GST_STREAM_ERROR_NOT_IMPLEMENTED)

        if error_domain == self.gst.core_error_quark():
            return error_code == GST_CORE_ERROR_MISSING_PLUGIN
    ...
    def _handle_message(self, msg: ctypes.c_void_p) -> Optional[int]:
        ...
        if msg_type & GST_MESSAGE_EOS:
            ...
            if self._is_not_supported_error(error.domain, error.code):
