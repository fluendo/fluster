...
class GstCtypes:
    ...
    def message_parse_error(self, message: ctypes.c_void_p) -> GstMessageError:
        ...
        if error.value:
            gerror = ctypes.cast(error.value, ctypes.POINTER(GErrorStruct)).contents
            error_domain = int(gerror.domain)
            error_code = int(gerror.code)
            if gerror.message:
                error_msg = gerror.message.decode("utf-8", errors="replace")
        ...
        return GstMessageError(error_domain, error_code, error_msg, debug_msg)
