...
def interleave_pcm_files(pcm_files: List[str], output_filepath: str) -> None:
    ...
    remaining_channels = [
        f
        for f in pcm_files
        if f not in front_channels and f not in side_channels and f not in back_channels and f not in lfe_channels
    ]
    sorted_files += sorted(remaining_channels)
    ...
    print(f"Interleaved PCM files to {output_filepath}:")
