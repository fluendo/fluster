...
def assert_fluster_success(args: List[str], no_emoji: bool = False) -> subprocess.CompletedProcess[str]:
    result = run_fluster(args, no_emoji=no_emoji)
    if result.returncode != 0:
        raise AssertionError(f"fluster command failed: {args}\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}")
    return result
...
class TestReference(unittest.TestCase):
    def setUp(self) -> None:
        assert_fluster_success(["download", "dummy", "-k"])
...
class TestRun(unittest.TestCase):
    def setUp(self) -> None:
        assert_fluster_success(["download", "dummy", "dummy_fail", "-k"])
    ...
    def test_run_h264_decoders(self) -> None:
        ...
        result = run_fluster(["run", "-ts", "H264-min", "-d", "GStreamer-H.264-Libav", "FFmpeg-H.264", "-s"])
    ...
    def test_run_h265_decoders(self) -> None:
        ...
        result = run_fluster(["run", "-ts", "H265-min", "-d", "GStreamer-H.265-Libav", "FFmpeg-H.265", "-s"])
        # May fail if decoders are not available, but command should run
